...
export async function PUT(
  ...
  // ── orphan Storage 정리는 저장 경로에서 제거 ───────────────
  // 과거엔 여기서 즉시 cleanup을 돌렸으나, 방금 올라온(또는 올라오는 중인) 파일을
  // 미참조 orphan으로 오인해 삭제하는 레이스가 있었다. 이제 lifecycle 크론의
  // sweepOrphans()가 보호 구간을 두고 일괄 회수한다. (lib/storage-cleanup.ts)
