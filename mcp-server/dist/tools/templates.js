...
export function registerTemplates(server) {
    // List templates
    server.tool('list_templates', 'List available page templates (landing page, event invite, link-in-bio, etc.)', {}, { readOnlyHint: true }, async () => {
        ...
        const list = result.data.data.map((t) => `- **${t.id}**: ${t.name} — ${t.description} (${t.blockCount} blocks)`).join('\n');
        return {
            content: [{ type: 'text', text: `Available templates:\n\n${list}` }],
        };
    });
    // Create from template
    server.tool('create_page_from_template', 'Create a new page from a template. You can override individual block content by index.', {
    ...
    }, { readOnlyHint: false, destructiveHint: false }, async ({ templateId, overrides }) => {
        const result = await createFromTemplate(templateId, overrides ?? undefined);
        if (!result.ok) {
            return {
                content: [{ type: 'text', text: `Error: ${result.error?.message ?? 'Creation failed'}` }],
                isError: true,
            };
        }
        return {
