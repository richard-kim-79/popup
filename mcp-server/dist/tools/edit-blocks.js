import { z } from 'zod';
import { editBlocks } from '../lib/api-client.js';
export function registerEditBlocks(server) {
    server.tool('edit_page_blocks', 'Partially edit blocks on a page using batch operations (insert, update, delete, move). More efficient than replacing all blocks.', {
    ...
    }, { readOnlyHint: false, destructiveHint: false }, async ({ slug, operations }) => {
        const result = await editBlocks(slug, operations);
        if (!result.ok) {
            return {
                content: [{ type: 'text', text: `Error: ${result.error?.message ?? 'Edit failed'}` }],
                isError: true,
            };
        }
        return {
...
//# sourceMappingURL=edit-blocks.js.map