...
export function registerUploadImage(server: McpServer) {
  server.tool(
    'upload_image',
    ...
    async ({ slug, filename, mimeType, size }) => {
      const result = await requestUpload(slug, filename, mimeType, size)
      if (!result.ok) {
        return {
          content: [{ type: 'text' as const, text: `Error: ${result.error?.message ?? 'Upload request failed'}` }],
          isError: true,
        }
      }
